...
def create_app():
    ...
    # JSON error handlers — prevent Flask/Werkzeug from returning HTML error pages
    ...
    def forbidden(e):
        return jsonify({'error': 'Permission denied', 'authorized': False}), 403

    @app.errorhandler(404)
    def not_found(e):
        # Return HTML for non-API routes (frontend SPA), JSON for API routes
        if request.path.startswith('/api'):
